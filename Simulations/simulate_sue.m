...
model_master_dir = '../Models/Sue_Inspired_Models/';
category = 'Sue_Leg_BranchCount_2';
model_name_prefix = 'Sue_leg';
model_name_suffix_arr = 2;
result_master_dir = 'Results/Sue_leg/';
...
for i = 1 : numel(model_name_suffix_arr)
    model_name = [model_name_prefix, num2str(model_name_suffix_arr(i))];
    
    individual_model_dirs{end+1, 1} = fullfile(model_master_dir, category);
    individual_model_names{end+1, 1} = model_name;

    individual_result_dirs{end+1, 1} = fullfile(result_master_dir, 'Sue_leg_2');
end
...
prescribed_controller_params.exc_starttime = 0.2;
prescribed_controller_params.exc_endtime = 0.8;
prescribed_controller_params.exc_startvalue = 0;
prescribed_controller_params.exc_endvalue = 0.05;

% Only stimulate one selected MTU
prescribed_controller_params.mtu_names = {'trex_flexor_foot'};
...
for i = 1 : numel(individual_model_dirs)
    ...
    for j = 1 : numel(explicit_branching)
        ...
        save([result_save_dir, individual_model_names{i}, '_SimData.mat'], 'coord_report');
